...
from sidebar import render_roster
...
try:
    ...
    st.error("⚠️ Lost connection to the database. This page will retry automatically in a few seconds — your data is safe, please don't refresh manually.")
    st.stop()

touch_user_last_seen(st.session_state.username)

# ---------------- GLOBAL ANNOUNCEMENT ----------------
if db.get("announcement"):
    st.markdown(
        f"<div class='announce-banner'>🚨 {db['announcement']}</div>",
